...
function db_ping($type, $host, $login, $passwd, $dbname) {
    try {
        $db = new PDO("$type:host=$host;dbname=$dbname", $login, $passwd);
    }
...
function db_prepare() {
    ...
    db_statement_prepare('course_create',
    ...
    db_statement_prepare('course_read',
        'SELECT ' . 
            db_gettable('courses') . '.course_code, ' .
            db_gettable('courses') . '.course_name, ' .
    ...
    db_statement_prepare('course_get_users', 
        'SELECT ' .
    ...
    db_statement_prepare('users_get_admins',
        'SELECT user_ID '.
        'FROM ' .  db_gettable('users').' ' .
        'WHERE permissions > 0'
    );
    
    db_statement_prepare('get_users_in_recorder', 
        'SELECT DISTINCT '.
            db_gettable('users').'.user_ID, '.
    ...
    db_statement_prepare('get_internal_users',
        'SELECT ' . 
            db_gettable('users') . '.user_ID, ' .
            db_gettable('users') . '.surname, ' .
            db_gettable('users') . '.forename, ' .
...
function db_user_get_courses($user_ID) {
    ...
    $statements['user_get_courses']->bindParam(':user_ID', $user_ID);
    $statements['user_get_courses']->execute();
    
    return $statements['user_get_courses']->fetchAll();
}
