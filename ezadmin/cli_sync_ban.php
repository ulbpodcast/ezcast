...
function ban_ez_users_cmp($ban,$ez){
    ...
    if(strtolower($ban['netID'])==strtolower($ez['user_ID'])){
      ...
      if(strtolower($ban['NOM'])==strtolower($ez['surname']) && strtolower($ban['PRENOM'])==strtolower($ez['forename']) && $ez['origin']=="external" && strtoupper($ez['surname'])!=$ez['surname'] )
        ...
        $result['data']=1; //not same data or uppercase name in ezcast or origin internal to become external
    }
    else if(strtolower($ban['netID']) > strtolower($ez['user_ID'])){
...
//COURSES comparison functions (needed to do a diff)
 function ban_ez_courses_cmp($ban,$ez){
   ...
   /* print "\ncomparing:\n";
    ...
    else if(strtolower($ban['MNEMONIQUE']) > strtolower($ez['course_code'])){
...
function ban_ez_users_courses_cmp($ban,$ez){
    ...
    else if(strtolower($ban['MNEMONIQUE'])>strtolower($ez['course_code']) || (strtolower($ban['MNEMONIQUE'])==strtolower($ez['course_code']) && strtolower($ban['netID']) >strtolower($ez['user_ID']))){
            $result['key']=1; //ban netid greater
             //print "key1 > key2\n";
      }else{
            $result['key']=2; //ban netid smaller
...
function ez_users_courses_delete($ez){
 ...
 if($ez['origin']=="external")
     ...
     $sql="";//keep internal user_courses 
return $sql;
 }
...
function ban_ban_users_courses_cmp($ban_courses,$ban_users){
    ...
    else if(strtolower($ban_courses['MNEMONIQUE']) > strtolower($ban_users['MNEMONIQUE'])){
            $result['key']=1; //ban netid greater
             //print "key1 > key2\n";
      }else{
            $result['key']=2; //ban netid smaller
...
function next1(&$array=array(),$reset=false){
   ...
   if($idx>=count($in_array))return false;
   /*print "next1 returns:";
