...
function index($param = array())
{
    ...
    if (isset($input['post'])) {
        ...
        $has_albums = (isset($input['has_albums'])&& $input['has_albums']) ? 1 : 0;
        $no_albums = (isset($input['no_albums'])&& $input['no_albums']) ? 1 : 0;
        $in_recorders = (isset($input['in_recorders'])&& $input['in_recorders']) ? 1 : 0;
        $out_recorders = (isset($input['out_recorders'])&& $input['out_recorders']) ? 1 : 0;
        $with_teacher = (isset($input['with_teacher'])&& $input['with_teacher']) ? 1 : 0;
        $without_teacher = (isset($input['without_teacher'])&& $input['without_teacher']) ? 1 : 0;
        $page = $input['page'];
        $col = $input['col'];
        $order = $input['order'];
        $size = 20;
