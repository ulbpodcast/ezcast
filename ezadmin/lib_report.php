...
class Report {
    ...
    private function repository_get_all() {
        ...
        foreach($listFile as $filePath) {
            ...
            $output_array = array();
            preg_match("/".preg_quote(self::EZCAST_FOLDER.self::REPOSITOR_FOLDER, '/')."\/(([\w-_]*)\-[^\-^\/]*)\/(\w*)\/_metadata.xml/", 
            ...
            if($this->param_general) {
                $this->calcul_general_infos($xml, $this->count_total_asset, $this->count_submit_asset,
                    $this->count_record_asset, $this->list_all_author, $this->list_all_submit_author, 
                    $this->list_all_record_author, $this->list_all_cours, $this->list_all_cours_submit, 
            ...
            if($this->param_start_date <= $nbrDate && $nbrDate <= $this->param_end_date) {
                ...
                $this->calcul_general_infos($xml, $this->date_count_asset, $this->date_count_submit_asset, 
                    $this->date_count_record_asset, $this->date_list_author, 
                    $this->date_list_submit_author, $this->date_list_record_author,
                    $this->date_list_cours, $this->date_list_cours_submit, 
                    $this->date_list_cours_record, $this->date_classroom_record_time);
            }

        }
    }

    private function calcul_general_infos(&$asset, &$countAsset, &$countSubmitAsset, 
        &$countClassroomAsset, &$listAuthor, &$submitAuthor, &$recordAuthor,
        &$listCours, &$listCoursSubmit, &$listCoursRecord, 
    ...
    private function repository_get_xml($folder, &$res = array(), $rec = 0) {
        ...
        foreach(scandir($folder) as $file) {
            if (!in_array($file, self::$IGNORED_FILES))  {
                ...
                } else if($file == self::META_FILE_NAME && $rec == 2) {
                    $res[] = $folder.'/'.$file;
    ...
    private function trace_info_bookmark_add(&$traceInfo, &$newEntryTrace, $isInDate) {
        ...
        $newEntryTrace = array_merge($newEntryTrace, 
    ...
    function get_list_all_author() {
        return $this->list_all_author;
    }
    
    function get_nbr_list_all_author() {
        return count($this->get_list_all_author());
    }
    
    function get_list_all_submit_author() {
        return $this->list_all_submit_author;
    }
    
    function get_nbr_list_all_submit_author() {
        return count($this->get_list_all_submit_author());
    }
    
    function get_list_all_record_author() {
        return $this->list_all_record_author;
    }
    
    function get_nbr_list_all_record_author() {
        return count($this->get_list_all_record_author());
    }
    
    function get_list_all_cours() {
        return $this->list_all_cours;
    }
    
    function get_nbr_list_all_cours() {
        return count($this->list_all_cours);
    }
    
    function get_list_all_cours_submit() {
        return $this->list_all_cours_submit;
    }
    
    function get_nbr_list_all_cours_submit() {
