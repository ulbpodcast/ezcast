...
if(!ezmam_asset_exists($album, $asset))myerror ("ezmam did not find referenced asset: $album, $asset");
...
if(!is_array($resultprocessing_assoc))myerror ("could not get result metadata");
$status=$resultprocessing_assoc['status'];
if($status!="processed")myerror ("processing return status: $status");
...
if(isset($duration) && is_numeric($duration))$asset_meta['duration']=round($duration);
$asset_meta['status']='processed';
$res=ezmam_asset_metadata_set($album, $asset, $asset_meta);
if(!$res)print "asset metadata set error:".ezmam_last_error ()."\n";
...
function high_low_media_mam_insert($album,$asset,$high_low,$cam_slide,$processing_assoc,$render_dir){
   ...
   if(!is_array($qtinfo_assoc))myerror ("error in intro-title-movie {$high_low}_{$cam_slide}_qtinfo.xml missing/corrupted");
   ...
   if(isset($qtinfo_assoc['duration']))$duration=$qtinfo_assoc['duration'];
   ...
   if(!isset($duration))$duration=$qtinfo_assoc['duration'];
    ...
    print "insert media $album $asset $media\n";
     ...
     if(!$res)myerror ("could not add media to repository: file $media_file_path");
...
function update_originals_meta($album,$asset,$qtinfo_filepath){
 ...
 $media_meta['height']=$qtinfo_assoc['height'];
 $media_meta['width']=$qtinfo_assoc['width'];
 if(isset($qtinfo_assoc['videoCodec']))$media_meta['videocodec']=$qtinfo_assoc['videoCodec'];
 if(isset($qtinfo_assoc['audioCodec']))$media_meta['audiocodec']=$qtinfo_assoc['audioCodec'];
