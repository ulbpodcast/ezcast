...
$course_code_public = isset($album_metadata['course_code_public']) ? $album_metadata['course_code_public'] : suffix_remove($album);
...
if (isset($asset_metadata['add_title']) && $asset_metadata['status'] == 'processed') { 
    ...
    $cmd3 = 'ssh '.$ezrenderer['client'].'@'.$ezrenderer['host'].' "'.$php_cli_cmd.' '.$ezrenderer['processed_dir'].'/../../bin/gen_new_intro.php '.$ezrenderer['processed_dir'].'/../processing/'.$album.'_'.$asset.' \"'.base64_encode($album_metadata['name']).'\"  \''.base64_encode($copyright).'\'  \''.base64_encode($organization_name).'\'  \''.base64_encode($course_code_public).'\' "';
    ...
    exec("$cmd2 && $cmd3 && $cmd4 && $cmd5" , $cmdoutput2, $returncode2 );
