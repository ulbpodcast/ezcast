...
function index($param = array()) {
    ...
    if (!acl_has_album_permissions($album)) {
        error_print_message(template_get_message('Unauthorized', get_lang()));
         $logger->log(EventType::MANAGER_MODO_DELETE, LogLevel::WARNING, 'User tried to delete modo '.$iduser.' from album  '. $album. ' but is not authorized', 
                 array(basename(__FILE__)), $album);
        die;
    }
