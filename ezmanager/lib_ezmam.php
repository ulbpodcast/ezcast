...
if($repository_path===false){
  ...
  if(ezmam_album_exists($album_name))return "album already exists";
  $res=mkdir($album_path);
  if(!$res)return "could not create $album_path";
  
  // Create a token
  $res=ezmam_album_token_create($album_name);
  if(!$res) return "could not create token for $album_path";

  //create the metadata
  $res=ezmam_album_metadata_set($album_name,$metadata);
  if(!$res) return "could not create metadata for $album_path";
...
if($repository_path===false){
 ...
 while (($file = readdir($dh)) !== false) {
     if($file[0]!='.' && $file[0]!="_"){ //filter out names starting with . (including '.' and '..' )or _
       if(is_dir($repository_path ."/". $file)) array_push ($album_list, $file); //if its a directory add it to the list
      }
 }//end while
 return $album_list;
}
...
function ezmam_album_metadata_get($album){
  ...
  if(!ezmam_album_exists($album))return false;
...
function ezmam_rss_new($album_name, $type) {
  ...
  if(!ezmam_album_exists($album_name))return false;
  $album_path=$repository_path."/".$album_name;
...
function ezmam_asset_list($album){
 ...
 //foreach($dh as $file) {
     if($file[0]!='.' && $file[0]!="_"){ //filter out names starting with . (including '.' and '..' )or _
       if(is_dir($album_path ."/". $file)) array_push ($asset_list, $file); //if its a directory add it to the list
      }
 }//end while
 return $asset_list;
}
...
function ezmam_asset_metadata_get($album,$asset){
  ...
  if(!ezmam_album_exists($album))return false;
  if(!ezmam_asset_exists($album,$asset)) return false;
...
if($repository_path===false){
  ...
  if(!ezmam_album_exists($album_name))return "album non existant";
  if(ezmam_asset_exists($album_name,$asset_name))return "asset already exists";
  ...
  if(!$res) return "could not create token for $asset_path";
...
function ezmam_media_list($album,$asset){
 ...
 while (($file = readdir($dh)) !== false) {
     if($file[0]!='.' && $file[0]!="_"){ //filter out names starting with . (including '.' and '..' )or _
       if(is_dir($asset_path ."/". $file)) array_push ($media_list, $file); //if its a directory add it to the list
      }
 }//end while
 return $media_list;
}
...
function ezmam_media_metadata_get($album,$asset,$media){
  ...
  if(!ezmam_album_exists($album))return false;
  if(!ezmam_asset_exists($album,$asset)) return false;
  if(!ezmam_media_exists($album,$asset,$media)) return false;
...
if($repository_path===false){
  ...
  if($copy){
    if(is_dir($media_file_path)){//media can be a directory (e.g in case of a chapter list+images)
       ...
       else $res=true; //returncode==0 so recursive copy went well
    }else{
...
// TODO: Implement this function (not working ftm)
...
function ezmam_media_delete($album_name, $asset_name, $media_name){
    ...
    while(($file = readdir($dir)) !== false) {
        if($file != "." && $file != "..") {
            if(is_dir($path.'/'.$file))
              exec('rm -rf '.$path.'/'.$file);//chapterslide media are directories so we need to delete it recursively
             else
              unlink($path.'/'.$file); //single file media
...
$token="";
for($idx=0;$idx<8;$idx++)$token.= chr(rand(65,90));
