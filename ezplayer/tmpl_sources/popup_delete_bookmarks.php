...
<div id="popup_delete_bookmarks" class="reveal-modal left up">
    ...
    <?php if (isset($album_bookmarks)){ 
        // delete album bookmarks
        $bookmarks = $album_bookmarks; 
        $source = 'assets';
    } else { 
        // delete asset bookmarks
        $bookmarks = $asset_bookmarks; 
        $source = 'details';        
    }?>
    <?php if (isset($bookmarks) && count($bookmarks) > 0){ ?>
    <form action="index.php?action=delete_bookmarks" method="post" id="select_delete_bookmark_form" name="delete_bookmarks_form" onsubmit="return false">
        <input type="hidden" name="album" id="delete_album" value="<?php echo $album; ?>"/>
        <input type="hidden" name="asset" id="delete_asset" value="<?php echo $asset_meta['record_date']; ?>"/><br/>
        <ul>
            <li style="border-bottom: solid 1px #cccccc;"><input type="checkbox" onclick="toggle_checkboxes(this, 'delete_selection[]')" name="check_all"/><span class="blue-title"><b>®Date®</b></span><span class="blue-title"><b>®Bookmark®</b></span></li>
        ...
        <a href="#" onclick="submit_delete_bookmarks_form('<?php echo $source; ?>');" id="delete_button" class="delete-button-confirm" title="®Delete_selected_bookmarks®">®Delete®</a>
