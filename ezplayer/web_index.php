...
function bookmarks_search() {
    ...
    /*
      ...
      if ($target != 'current' && !acl_has_album_permissions($album)){
      // happens when the user has access to one asset but does not have access to the rest of the album
      $current_toc = user_prefs_bookmarks_search($_SESSION['user_login'], $search, "current", $albums, $fields, $level, 'official');
      }
      $bookmarks_toc = user_prefs_bookmarks_search($_SESSION['user_login'], $search, $target, $albums, $fields, $level, 'official');

      if (is_array($current_toc) && is_array($bookmarks_toc)){
      // both arrays have results, we merge them
      $bookmarks_toc = array_merge($current_toc, $bookmarks_toc);
      } else if (is_array($current_toc)){
      // only current asset has results
      $bookmarks_toc = $current_toc;
      }    }
      if (in_array('custom', $tab)) {
      $bookmarks = user_prefs_bookmarks_search($_SESSION['user_login'], $search, $target, $albums, $fields, $level);
      }
      if (in_array('threads', $tab)) {
      $search_result_threads = thread_search($search, $target, $albums, $fields_thread);
      }
     */
 ...
 * @return boolean
 */
function asset_edit_meta() {
    global $input;
    $album = $input['album_download_link'];
    $asset = $input['asset_download_link'];
    $display_download_link = $input['display_download_link'];
    asset_meta_update($album, $asset, "display_download_link", $display_download_link);
