...
if(isset($title_assoc['add_title']) && $title_assoc['add_title'] !='false' && $title_assoc['add_title'] !=''){
	...
	if(isset($title_assoc['title_time']) && $title_assoc['title_time'] !='') $title_time=$title_assoc['title_time'];
	if(isset($title_assoc['intro']) && $title_assoc['intro']!=""){
		print "\n------------------------ Intro exists------------------------\n";
		if(isset($title_assoc['intro_time']) && $title_assoc['intro_time']!='') $duration_intro=$title_assoc['intro_time'];//TO DO : implémenter a la creation de l'asset
		else{
			...
			else $duration_intro=$default_intro_duration;
		}		
...
function create_intro_parts($processing,$record_types,$duration_intro,$title_time,$hasintro){
	...
	for($i=0;$i<count($record_types);$i++){
		if(is_dir($processing.'/'.$record_types[$i])){
			if($hasintro) $cmd=' '.$ffmpegpath.' -i '.$processing.'/'.$record_types[$i].'/'.$record_types[$i].'.mov -vcodec copy -acodec copy -ss 00:00:00 -t '.$duration_intro.' -y '.$processing.'/'.$record_types[$i].'/'.$record_types[$i].'1.mov  -vcodec copy -acodec copy -ss '.($duration_intro+$title_time).' -y '.$processing.'/'.$record_types[$i].'/'.$record_types[$i].'2.mov';
			else $cmd=' '.$ffmpegpath.' -i '.$processing.'/'.$record_types[$i].'/'.$record_types[$i].'.mov -vcodec copy -acodec copy -ss '.($title_time).' -y '.$processing.'/'.$record_types[$i].'/'.$record_types[$i].'2.mov';
			exec($cmd,$cmdoutput,$returncode);
...
function create_list($processing,$record_types,$hasintro){
	...
	for($i=0;$i<count($record_types);$i++){
		if(is_dir($processing.'/'.$record_types[$i]) && file_exists($processing."/".$record_types[$i]."/title.mov")  ){
			...
			else if(file_exists($processing."/".$record_types[$i]."/title.mov") && file_exists($processing."/".$record_types[$i]."/".$record_types[$i]."2.mov") ){
