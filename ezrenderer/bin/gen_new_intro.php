...
//$title_assoc['add_title'] may not exists for older records
...
"juillet", "août", "septembre", "octobre", "novembre", "décembre");
    ...
    if (isset($title_assoc['intro']) && $title_assoc['intro'] != "" && file_exists($intros_dir.'/'.$title_assoc['intro'].'/original_intro.mov')) {
        ...
        if($duration_intro === null) {
            echo "Could not get intro duration" . PHP_EOL;
            exit(3);
        }
    ...
    for($i = 0;$i<count($record_types);$i++){
        if (is_dir($processing.'/'.$record_types[$i])) {
            if ($error == 0) {	
                ...
                print("\n Error occured, unable to finish creation of new intro \n"); 
            }
