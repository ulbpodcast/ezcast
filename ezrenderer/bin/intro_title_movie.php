...
function itm_intro_title_movie($camslide, $moviein, &$title_assoc, $intro, $add_title) {
    ...
    foreach ($qualities as $quality) {
        ...
        $transcoded_movie = itm_handle_movie($moviein, $camslide, $quality, $encoder);
        $dt = time() - $t1;
        ...
        if (trim($intro) != "") {
            ...
            switch($qtinfo["aspectRatio"]){
                case "16:9":                    
                    ...
                    if (!file_exists($intro_movie)) $intro_movie = $intros_dir . "/$intro" . "/" . $intro_movies['default'];
                    break;
                case "16:10":                    
 ...
 */
function itm_handle_movie($movie, $camslide, $quality, &$encoder) {
    ...
    if ($quality == 'high') {
        ...
        $vididx = 0;
        $count = count($accepted_video_sizes);
        while ($vididx < $count && $width > $accepted_video_sizes[$vididx]) {
            $vididx += 1;
        }
        $vididx = ($vididx == $count) ? $vididx - 1 : $vididx;
        $good_width = $accepted_video_sizes[$vididx];
        $encoder = $video_high_transcoders[(string) $good_width];
        print "\nSize: ${width}x$height 
               \nQuality: $quality
               \nGood_width: $good_width
               \nEncoder: $encoder\n";
    } else if ($quality == 'low') {
        $encoder = $video_high_transcoders['low'];
    } else {
        $encoder = $video_high_transcoders['super_highres'] . $width . 'x' . $height;
    }
    ...
    $res = safe_movie_encode($movie, $movieout, $encoder, $qtinfo);
    if ($res)
