...
function sync_video($movies_path){
	...
	if($campath!='' && !file_exists($campath.".wav")){
		...
		else print "Audio creation of ".$campath.".wav from ".$campath." SUCCEED. \n";
	}
	...
	if(file_exists($campath.".wav") && file_exists($slidepath.".wav") && file_exists($slidepath) && file_exists($campath)){
		...
		else print "Get diff_time between the two audio files failed.  cmd: ".$cmd." \n";
		...
		else print "Get duration of ".$campath." failed.  cmd: ".$cmd." \n";
		...
		if($return!= 0) print "DELETE ".$campath.".wav failed. \n";
		exec('rm '.$slidepath.'.wav ',$err,$return);
		if($return!= 0) print "DELETE ".$slidepath.".wav failed. \n";
