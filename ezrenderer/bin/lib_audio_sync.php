    ...
    if (file_exists($campath.".wav") && file_exists($slidepath.".wav") && file_exists($slidepath) && file_exists($campath)){
        ...
        if (!is_null($duration) && !is_null($diff_time) && ($duration-$diff_time) >0 && $diff_time<20 ){				
            ...
            if ($copy_sound_from_slide_to_cam){
                ...
                //Delete cam sound
                $cmd = $ffmpegpath.' -i '.$campath.' -an -vcodec copy -acodec copy  '.$movies_path.'/camNosound.mov';
                exec($cmd,$err,$return);
                if ($return != 0) 
                    $error = 1;    
                
                //insert slide sound into cam movie
                $cmd = $ffmpegpath.' -i '.$movies_path.'/camNosound.mov'.' -i '.$movies_path.'/slidesound.mp3'. ' -shortest -y -vcodec copy -acodec copy '.$campath;
                exec($cmd,$err,$return);
                if ($return != 0) 
                    $error = 1;   
